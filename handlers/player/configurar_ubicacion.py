...
def handle_address_configuration(message: Message, bot: TelegramBot):
    ...
    bot.register_next_step_handler(msg, process_address_step, bot, user_public_id)
...
def process_address_step(message: Message, bot: TelegramBot, user_public_id: UUID):
    ...
    response = players_service.update_partial_player(user_public_id, player_partial_data)
    if response:
        ...
        bot.register_next_step_handler(msg, process_radius_step, bot, user_public_id)
    else:
