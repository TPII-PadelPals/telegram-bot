...
def handle_configure_strokes(message: Message, bot: TeleBot, get_api=get_from_env_api, get_len=get_from_env_lang):
    ...
    if text.strip() == "/configurar_golpes" or len(info_list) != EXPECTED_INFORMATION:
        bot.reply_to(message, language["MESSAGE_HELP_STROKE"])
        return
    hability = info_list[POSITION_OF_HABILITY].lower()
    # caso de error en habilidad
    if not hability in language["STROKE_HABILITY"]:
        bot.reply_to(message, language["MESSAGE_INCORRECT_HABILITY"])
        return
    # obtengo el listado de golpes a configurar
    if info_list[POSITION_OF_STROKES].lower() == language["ALL"].lower():
    ...
    response_to_user = language["MESSAGE_STROKES_UPDATED"]
